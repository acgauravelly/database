...
//            System.setProperty("webdriver.gecko.driver", System.getProperty("user.dir") + "\\Mozilla\\geckodriver.exe");
    ...
    public void dropList(String objectName, String text) {
        ...
        WebElement droplist = driver.findElement(By.cssSelector(OR.getProperty(objectName)));
...
//        driver.findElement(By.cssSelector(objectName));
        WebDriverWait wait = new WebDriverWait(driver, 30);
        wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.cssSelector(OR.getProperty(objectName))));
    }

    public void expWaitVisible(String objectName) {
        driver.findElement(By.cssSelector(objectName));
        WebDriverWait wait = new WebDriverWait(driver, 30);
        wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.cssSelector(OR.getProperty(objectName))));
    }

    //	Waits for 120 secs for a text to appear in an object by cssSelector
    public void waittilltextPresent(String objectName, String text) {
        WebDriverWait wait = new WebDriverWait(driver, 60);
        wait.until(ExpectedConditions.textToBePresentInElementLocated(By.cssSelector(OR.getProperty(objectName)), text));
    }
    ...
    public void ClickChecboxByName(String checkBoxValue) {
        List<WebElement> lc = driver.findElements(By.cssSelector(OR.getProperty("checkBox")));
    ...
    public void clickCheckboxifNotSelected(String objectName) {
        log("clicking on" + "-----" + objectName);
        WebElement checkBox = driver.findElement(By.cssSelector(OR.getProperty(objectName)));
    ...
    public void checkboxSixe(String objectname) {
        List<WebElement> lc = driver.findElements(By.cssSelector(OR.getProperty(objectname)));
        for (int i = 0; i <= lc.size(); i++) {
    ...
    public void UncheckAllCheckBoxes() {
        ...
        List<WebElement> lc = driver.findElements(By.cssSelector("//input[@ type='checkbox']"));
    ...
    public void verifyVideoProductsPresent(String objectName) throws InterruptedException {
        Thread.sleep(4000);
        WebElement searchlist = driver.findElement(By.cssSelector(OR.getProperty(objectName)));
        List<WebElement> options = searchlist.findElements(By.cssSelector("//td[@aria-describedby='list47_assets']"));
        System.out.println(options.size());
        for (int i = 0; i < options.size(); i++) {

            Dimension size = options.get(i).getSize();
            System.out.println(size);
//			WebElement Videoicons=driver.findElement(By.cssSelector(OR.getProperty("VideoProducts")));

//			options.get(i).findElement(By.cssSelector(OR.getProperty("VideoProducts"))).
    ...
    public void verifyFieldsContainsText(String objName, String objectName, String brandName) throws InterruptedException {
        Thread.sleep(5000);
        WebElement Searchlist = driver.findElement(By.cssSelector(OR.getProperty(objName)));
        List<WebElement> options = Searchlist.findElements(By.cssSelector(OR.getProperty(objectName)));
        System.out.println(options.size());
        for (int i = 0; i < options.size(); i++) {
            System.out.println(options.get(i).getText());
    ...
    public void ChooseFromList(String text) throws InterruptedException {
        Thread.sleep(3000);
        WebElement mpnList = driver.findElement(By.cssSelector(OR.getProperty("dropDown")));
    ...
    public void Verify(String objectName, String text) throws InterruptedException {
        Thread.sleep(3000);
        WebElement title = driver.findElement(By.cssSelector(OR.getProperty(objectName)));
    ...
    public void assertElementhasText(String objectName, String text) throws InterruptedException {
        ...
        System.out.println(driver.findElement(By.cssSelector(OR.getProperty(objectName))).getText());
        Assert.assertTrue(driver.findElement(By.cssSelector(OR.getProperty(objectName))).getText().contains(text));
    }
    ...
    public void verifyifonProduct_Page() {
        ...
        String expurl = "https://manage.flix360.com/product/info/id/947517/lang_id/1";
        String actUrl = driver.getCurrentUrl();
        System.out.println(actUrl);
        if (expurl != actUrl)
            driver.navigate().to("https://manage.flix360.com/product/info/id/947517/lang_id/1");
    ...
    public void delete_Asset_ifpresent() throws InterruptedException {
        if (isElementpresent("AssetCarousel")) {
            WebElement Assets = driver.findElement(By.cssSelector(OR.getProperty("AssetCarousel")));
...
// Now you can do whatever you need to do with it, for example copy somewhere
    ...
    public void readUnreadEmails() {
        ...
        List<WebElement> unreadEmails = driver.findElements(By.cssSelector(".zF"));

        String MyMailer = "prioritysupport";

        for (WebElement mail : unreadEmails) {
            if (mail.isDisplayed() && mail.getText().equalsIgnoreCase(MyMailer)) {
                emailRecieved = true;
            } else {
                Assert.fail("Email Not recieved");
            }
        }
    }

    public void switchTabsinBrowser() {
    ...
    public String randomMPN(){
        return "QA-Auto-Test-"+ UUID.randomUUID().toString();
    }
