...
//		driver.findElement(By.xpath(OR.getProperty(objectName))).clear();
    ...
    public boolean isElementDisplayed(String errormsg, String objectName) throws InterruptedException {

        try {
            Assert.assertTrue(errormsg, driver.findElement(By.xpath(OR.getProperty(objectName))).isDisplayed());
...
//		input("email", System.getProperty("username"));
//		input("password", System.getProperty("password"));
        click("signinButton");
    }
...
//			options.get(i).findElement(By.xpath(OR.getProperty("VideoProducts"))).
    ...
    public void DragandDropFiles() {
        WebElement Dropfield = driver.findElement(By.xpath(OR.getProperty("DragandDropField")));
        Dropfield.sendKeys("C:\\Users\\ajay\\Desktop\\Automation\\Assets\\Test1.jpeg");
        Dropfield.sendKeys("C:\\Users\\ajay\\Desktop\\Automation\\Assets\\Wildlife.wmv");
        Dropfield.sendKeys("C:\\Users\\ajay\\Desktop\\Automation\\Assets\\pdfTest.pdf");
    }

    //	click and add to uploaded assets
    public void ClickandAddtoAssets() throws InterruptedException {
        WebElement AssetsBox = driver.findElement(By.xpath(OR.getProperty("detailBox")));
        List<WebElement> Assets = AssetsBox.findElements(By.xpath(OR.getProperty("UploadedElements")));
        System.out.println(Assets.size());
        for (int i = 1; i < Assets.size(); i++) {
            Assets.get(i).click();
            Thread.sleep(5000);
            select("Format", "Brochure");
//			expWaitVisible("AddLanguage");
//			Boolean flag=isElementDisplayed("AddLanguage");
//			System.out.println(flag);
            click("AddLanguage");

            select("BULanguageField", "English");
            input("AssetTitle", "test");
            click("AddLanguageonpopup");
            input("DescriptionArea", "Test");
            Thread.sleep(3000);
        }
    }
...
// Switch to new window opened
        ...
        isElementDisplayed("element is not displayed", objectName);
...
//	check http status
...

    {
        ...
        int code = webClient.getPage(Config.getProperty(url)).getWebResponse().getStatusCode();
    ...
    public void getStatusCodeHub(String url) throws IOException
