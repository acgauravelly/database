...
public class BasePage {
    ...
    private String landingPageUrl = "http://cn.manage.flix360.com/search";
    ...
    private By deleteGmailButton = By.cssSelector(".ajT");
...
//    }
    ...
    public void verifyFieldsstartWithText(By locator, String text) throws InterruptedException {
        Thread.sleep(5000);
        WebElement Searchlist = findElement(searchList);
        List<WebElement> options = Searchlist.findElements(locator);
        System.out.println(options.size());
        if (options.size() != 0) {
            for (WebElement option : options) {
                System.out.println(option.getText());
                Assert.assertTrue("Brand name is not present in search results" + text + "******", option.getText().toLowerCase().startsWith(text));
            }
        } else {
            Assert.fail("Zero results returned");
        }
    ...
    public void readUnreadEmails() {
        ...
        for (WebElement mail : unreadEmails) {
            if (mail.isDisplayed() && mail.getText().equalsIgnoreCase(MyMailer)) {
                mail.click();
                if (isElementPresent(deleteGmailButton)) {
                    click(deleteGmailButton);
                }
            }
        }
    }
